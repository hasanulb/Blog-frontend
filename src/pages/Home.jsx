    ...
    <div className="bg-white min-h-screen">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        {sortedPosts.length > 0 && (
          <Link to={`/post/${sortedPosts[0]._id}`} className="grid md:grid-cols-2 gap-6 mb-10">
            <img
              src={sortedPosts[0].postImageUrl}
              alt={sortedPosts[0].title}
              className="w-full h-72 object-cover rounded-lg"
            />
            <div>
              <h1 className="text-2xl font-bold mb-2">{sortedPosts[0].title}</h1>
              <p className="text-gray-600 mb-3">{sortedPosts[0].description}</p>
              <div className="flex items-center gap-2 text-sm text-gray-500">
                <span>{sortedPosts[0].createdBy?.name || "Unknown"}</span>
